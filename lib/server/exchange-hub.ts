...
type ClientMessage =
  | { type: "subscribe"; exchange: Exchange }
  | { type: "unsubscribe"; exchange: Exchange }
  | { type: "ping"; ts: number }
  | { type: "sync" };
...
export class ExchangeHub {
  ...
  private exchangeStatus = new Map<
    Exchange,
  ...
  private handleClientMessage(session: ClientSession, msg: ClientMessage) {
    if (msg.type === "subscribe") {
      this.subscribe(session, msg.exchange);
    } else if (msg.type === "unsubscribe") {
      this.unsubscribe(session, msg.exchange);
  ...
  private connectExchange(exchange: Exchange) {
    ...
    const callbacks: AdapterCallbacks = {
      ...
      onStatusChange: (ex, status, latency) => {
        ...
        const msg = JSON.stringify({
          type: "status",
          data: { exchange: ex, status, latency },
        } satisfies ServerMsg);
