...
async function fetchBinanceInstruments(): Promise<InstrumentInfo[]> {
  ...
  for (const s of spotInfo.symbols as {
    symbol: string;
    status: string;
    quoteAsset: string;
  }[]) {
  ...
  const assetMap = new Map<
    string,
    { perpSymbol?: string; futures: FuturesContract[] }
  >();
...
async function fetchBybitInstruments(): Promise<InstrumentInfo[]> {
  ...
  for (const s of linearRes.result?.list ?? []) {
    ...
    if (s.contractType === "LinearPerpetual") {
      ...
      const expiry = new Date(parseInt(s.deliveryTime))
        .toISOString()
        .split("T")[0];
      entry.futures.push({
        symbol: s.symbol,
...
async function fetchOKXInstruments(): Promise<InstrumentInfo[]> {
  const [spotRes, swapRes, futuresRes, tickersRes] = (await Promise.all([
    ...
    { data?: Array<{ instId: string; state: string; quoteCcy: string }> },
    { data?: Array<{ instId: string; state: string; settleCcy: string }> },
    {
      data?: Array<{
        instId: string;
        state: string;
  ...
  const assetMap = new Map<
    string,
    { perpInstId?: string; futures: FuturesContract[] }
  >();
  ...
  for (const s of futuresRes.data ?? []) {
    ...
    const expiry = s.expTime ? parseOKXExpiry(s.expTime) : null;
    if (!expiry) continue;
  ...
  const results = await Promise.all(
    currencies.map(async (currency) => {
      const res = (await fetch(
        ...
        result?: Array<{ instrument_name: string; kind: string }>;
      };
      ...
      if (instruments.perpSymbol) {
        try {
          const ticker = (await fetch(
            ...
            result?: { stats?: { volume_usd?: number } };
          };
...
function parseOKXExpiry(expTimeMs: string): string | null {
  ...
  const timestamp = parseInt(expTimeMs);
  ...
  fetchInProgress = (async () => {
    ...
    for (const [name, settled] of [
      ...
      if (settled.status === "rejected") {
        console.warn(
          `[instruments] ${name} fetch failed:`,
          (settled.reason as Error)?.message ?? settled.reason,
        );
      }
    }
