...
export function calcSpotFuturesFeeAdjPnl(
  basisPercent: number,
  _makerFeePercent: number,
  takerFeePercent: number,
): number {
  // Round-trip fees for cash-and-carry arbitrage (4 legs):
  //   All trades executed as taker orders for immediate execution
  const totalFees = 4 * takerFeePercent;
 ...
 */
export function calcCalendarSpread(
  nearPrice: number,
  farPrice: number,
): number {
 ...
 * Assumes you pay taker on all 4 legs for immediate execution.
...
export function calcCalendarFeeAdjPnl(
  ...
  // All executed as taker for immediate execution
  const totalFees = 4 * takerFeePercent;
 ...
 */
export function calc1MinChange(
  currentPrice: number,
  ...
  const past = buffer.reduce(
    (closest, snap) => {
      if (snap.timestamp > oneMinAgo) return closest;
      if (
        !closest ||
        Math.abs(snap.timestamp - oneMinAgo) <
          Math.abs(closest.timestamp - oneMinAgo)
      ) {
        return snap;
      }
      return closest;
